...
class Purchase(models.Model):
    ...
    user_id = models.CharField(
        max_length=255, db_index=True, help_text="User/customer identifier"
    )
    product_id = models.CharField(max_length=255, help_text="Product identifier")
    quantity = models.IntegerField(
        default=1, help_text="Quantity of products purchased"
    )
    payment_id = models.CharField(
        max_length=255, help_text="Payment transaction identifier"
    )
    amount = models.DecimalField(
        max_digits=10, decimal_places=2, help_text="Purchase amount"
    )
    ...
    def __str__(self):
        return f"Purchase {self.transaction_id} - User {self.user_id} - {self.status}"
