...
class InventoryService:
    ...
    def _simulate_latency():
        if settings.SIMULATE_LATENCY:
            latency = random.uniform(
                settings.MIN_LATENCY_MS / 1000, settings.MAX_LATENCY_MS / 1000
            )
            time.sleep(latency)
    ...
    @staticmethod
    def _record_failure(operation_id, product_id, quantity, metadata=None):
    ...
    def decrease_stock(operation_id, product_id, quantity, metadata=None):
        ...
        """
        logger.info(
            f"Decrease stock request: operation_id={operation_id}, product_id={product_id}, quantity={quantity}"
        )
        ...
        if existing_op:
            if existing_op.status == "applied":
                ...
                raise InsufficientStockError(
                    f"Operation {operation_id} previously failed"
                )
        ...
        if InventoryService._should_simulate_no_stock():
            ...
            InventoryService._record_failure(
                operation_id, product_id, quantity, metadata
            )
            raise InsufficientStockError(
                f"Simulated: No stock for product {product_id}"
            )
        ...
        logger.info(
            f"Stock decreased successfully: product_id={product_id}, new_stock={inventory.stock}"
        )
    ...
    def compensate(operation_id, product_id, quantity, metadata=None):
        ...
        """
        logger.info(
            f"Compensate request: operation_id={operation_id}, product_id={product_id}, quantity={quantity}"
        )
        ...
        existing_comp = InventoryOperation.objects.filter(
            operation_id=operation_id, status="reverted"
        ).first()
        ...
        inventory, created = Inventory.objects.select_for_update().get_or_create(
            product_id=product_id, defaults={"stock": 0}
        )
        ...
        logger.info(
            f"Stock compensated successfully: product_id={product_id}, new_stock={inventory.stock}"
        )
